...
def predict_vote(dmsp, progress=gr.Progress(track_tqdm=True)):
    ...
    for input in progress_bar:
        ...
        predictions = torch.add(predicted, predictions)
        # print(predictions)
        # predictions.append(class_mapping[predicted.argmax(0)])
    probs = F.softmax(predictions, dim=1)
    class_probs = {class_mapping[i]: probs[0][i].item() for i in range(len(class_mapping))}
    print(class_probs)
...
interface = gr.Interface(fn=combined,
                         ...
                         # outputs= gr.Label(num_top_classes=11, label="Prediction"),
                         outputs=[gr.Video(label="Waveform"), gr.Label(num_top_classes=11, label="Prediction")],
